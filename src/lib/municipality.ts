...
function mapMunicipalityStatusToTenantStatus(status: string | null | undefined): Tenant["status"] {
  switch (status) {
    case "inactive":
      return "suspenso";
    case "implementation":
...
export function buildTenantSettingsFromMunicipality(
  ...
  return {
    ...
    site:
      ...
      base.site,
    secretariaResponsavel: municipality?.secretariatName || base.secretariaResponsavel,
    diretoriaResponsavel:
      (typeof general.directorship === "string" && general.directorship) || base.diretoriaResponsavel,
    diretoriaTelefone:
      (typeof general.directorship_phone === "string" && general.directorship_phone) || base.diretoriaTelefone,
    diretoriaEmail:
      (typeof general.directorship_email === "string" && general.directorship_email) || base.diretoriaEmail,
    horarioAtendimento:
      (typeof general.office_hours === "string" && general.office_hours) || base.horarioAtendimento,
    brasaoUrl: branding?.coatOfArmsUrl || base.brasaoUrl,
    logoUrl: branding?.logoUrl || base.logoUrl,
    resumoPlanoDiretor:
      (typeof general.resumo_plano_diretor === "string" && general.resumo_plano_diretor) || base.resumoPlanoDiretor,
    resumoUsoSolo:
      (typeof general.resumo_uso_solo === "string" && general.resumo_uso_solo) || base.resumoUsoSolo,
    leisComplementares:
      (typeof general.leis_complementares === "string" && general.leis_complementares) || base.leisComplementares,
    linkPortalCliente:
    ...
    registroProfissionalObrigatorio:
      settings?.requireProfessionalRegistration ?? base.registroProfissionalObrigatorio,
  };
}

export const buildInstitutionSettingsFromMunicipality = buildTenantSettingsFromMunicipality;
...
export function buildTenantFromMunicipalityBundle(
  ...
  const primary =
    ...
    fallbackTenant?.theme.primary ||
    "#0F2A44";
  const accent =
    branding?.accentColor ||
    fallbackTenant?.theme.accent ||
    "#5ee8d9";
  ...
  return {
    ...
    status: municipality ? mapMunicipalityStatusToTenantStatus(municipality.status) : fallbackTenant?.status || "ativo",
    plan:
    ...
    monthlyRevenue:
      ...
      0,
    subdomain: municipality?.subdomain || fallbackTenant?.subdomain || "",
    theme: {
      primary,
      accent,
    },
...
export const buildInstitutionFromMunicipalityBundle = buildTenantFromMunicipalityBundle;
