...
export function AppLoadingState({
  ...
  const tenantBranding = municipalityName
    ? {
        tenantId: municipalityName.toLowerCase().replace(/\s+/g, "-"),
        logoUrl: logoUrl || "",
        logoScale: 1,
        logoOffsetX: 0,
        logoOffsetY: 0,
          ...
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/12 ring-1 ring-white/10">
            {tenantBranding ? (
              <InstitutionalLogo
                ...
                fallbackLabel={municipalityName || "Prefeitura"}
                variant="compact"
                className="border-none bg-transparent p-0 shadow-none"
                viewportClassName="bg-white/96"
              />
            ) : (
