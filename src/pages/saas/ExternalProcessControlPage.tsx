...
import { formatCurrency, getProcessPaymentGuides, getVisibleProcessesByScope, statusLabel, statusTone } from "@/lib/platform";
...
export function ExternalProcessControlPage() {
  ...
  const requirementCount = processes.filter((process) =>
    (process.requirements ?? []).some((item) => item.status === "aberta" || item.status === "respondida"),
  ).length;
  const paymentCount = processes.filter(
    (process) => process.payment?.status === "pendente" || process.status === "pagamento_pendente" || process.status === "guia_emitida",
  ).length;
    ...
    <PortalFrame eyebrow="Acesso do profissional" title="Controle de processos">
      <PageContainer>
        <PageIntro
          ...
          description="Monitore etapas, pendências, pagamentos e movimentações com visão clara do que precisa de atenção."
          icon={TrendingUp}
          actions={
        ...
        <StatsCards>
          <MetricCard title="Em andamento" value={String(activeCount)} helper="Processos ativos" icon={FileBarChart2} tone="blue" />
          <MetricCard title="Com exigências" value={String(requirementCount)} helper="Pendências abertas" icon={Filter} tone="amber" />
          <MetricCard title="Aguardando pagamento" value={String(paymentCount)} helper="Guias pendentes" icon={Calendar} tone="rose" />
          <MetricCard title="Concluídos" value={String(completedCount)} helper="Processos encerrados" icon={FileCheck} tone="emerald" />
        </StatsCards>
        ...
        <MainGrid>
          <MainContent>
            <SectionPanel
              ...
              actions={
                <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_200px_220px_200px]">
                  <div className="relative min-w-[240px]">
                    <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <Input
                      value={search}
                      onChange={(event) => setSearch(event.target.value)}
                      placeholder="Buscar protocolo ou projeto"
            ...
            >
              <div className="hidden rounded-[10px] border border-[#E5E7EB] bg-[#F8FAFC] px-5 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 xl:grid xl:grid-cols-[1.2fr_0.85fr_0.85fr_0.75fr_0.8fr_0.75fr_0.75fr] xl:gap-3">
                <span>Protocolo</span>
                <span>Projeto</span>
                <span>Status</span>
              ...
              {filteredProcesses.map((process) => {
                ...
                const lastUpdateLabel = lastUpdate ? new Date(lastUpdate).toLocaleDateString("pt-BR") : "—";

                return (
                  <div key={process.id} className="sig-dark-panel rounded-[10px] border border-[#E5E7EB] p-4 shadow-sm">
                    <div className="grid gap-4 xl:grid-cols-[1.2fr_0.85fr_0.85fr_0.75fr_0.8fr_0.75fr_0.75fr] xl:items-center">
                      <div className="min-w-0">
                        <p className="sig-fit-title text-base font-semibold text-slate-950" title={process.protocol}>
                          {process.protocol}
                        </p>
                        <p className="sig-fit-copy mt-1 text-sm text-slate-500">{process.externalProtocol}</p>
                      </div>
                      <div className="min-w-0">
                        <p className="sig-fit-title text-sm font-semibold text-slate-900" title={process.title}>
                          {process.title}
                        </p>
                        <p className="sig-fit-copy mt-1 text-sm text-slate-500">{process.type}</p>
                      </div>
                      <div className="min-w-0">
                        <Badge variant="outline" className={`rounded-full capitalize ${statusTone(process.status)}`}>
                          {statusLabel(process.status)}
                        </Badge>
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-900">{process.sla?.currentStage || "Em andamento"}</p>
                        <p className="mt-1 text-xs text-slate-500">Etapa atual</p>
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-900">{lastUpdateLabel}</p>
                        <p className="mt-1 text-xs text-slate-500">Última movimentação</p>
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-900">
                          {pendingGuide ? formatCurrency(pendingGuide.amount) : "Pago"}
                        </p>
                        <p className="mt-1 text-xs text-slate-500">{pendingGuide ? "Aguardando" : "Compensado"}</p>
                      </div>
              ...
              {filteredProcesses.length === 0 ? (
                <div className="sig-dark-panel rounded-[12px] border border-dashed border-slate-300 p-8 text-center">
                  <p className="text-lg font-semibold text-slate-900">Nenhum processo encontrado</p>
          ...
          <SideContent>
            <SectionPanel
              ...
              description="Exigências que precisam de retorno ou documento complementar."
                ...
                filteredProcesses
                  ...
                  .map((process) => (
                    <div key={`req-${process.id}`} className="sig-dark-panel rounded-[10px] border border-[#E5E7EB] p-4 text-sm text-slate-700">
                      ...
                      <p className="mt-2 text-xs text-amber-600 dark:text-amber-400">Exigência pendente</p>
                    </div>
                  ))
              )}
            </SectionPanel>
            ...
            <SectionPanel
              ...
              description="Os protocolos atualizados mais recentemente."
              contentClassName="space-y-3"
            >
              {recentMovements.length === 0 ? (
                ...
                recentMovements.map((process) => (
                  <div key={`recent-${process.id}`} className="sig-dark-panel rounded-[10px] border border-[#E5E7EB] p-4 text-sm">
                    <p className="text-sm font-semibold text-slate-900">{process.protocol}</p>
                    <p className="mt-1 text-sm text-slate-600 line-clamp-2">{process.title}</p>
                    <p className="mt-2 text-xs text-slate-500">Etapa: {process.sla?.currentStage || "Em andamento"}</p>
                  </div>
                ))
              )}
            </SectionPanel>

            <SectionPanel title="Ações rápidas" description="Atalhos úteis para o dia a dia do profissional." contentClassName="space-y-3">
