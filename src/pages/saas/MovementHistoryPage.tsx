...
export function MovementHistoryPage() {
  ...
  const renderDispatchList = (rows: typeof dispatchRows, emptyText: string, compact?: boolean, hideSelection?: boolean) =>
    rows.length === 0 ? (
      <div className="sig-dark-panel rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-600">{emptyText}</div>
    ) : (
      <div className="space-y-3">
        {rows.map((item) => (
          <div key={item.id} className="sig-dark-panel rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className={`mt-3 grid gap-3 ${compact ? "md:grid-cols-3" : "md:grid-cols-4"}`}>
              <div className="sig-dark-panel rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="sig-label">Prazo</p><p className="mt-2 text-sm font-medium text-slate-900">{item.dueDate}</p></div>
              <div className="sig-dark-panel rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="sig-label">Pasta atual</p><p className="mt-2 sig-fit-title break-words text-sm font-medium text-slate-900">{item.currentFolder}</p></div>
              <div className="sig-dark-panel rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="sig-label">Visibilidade</p><p className="mt-2 text-sm font-medium text-slate-900">{item.externalTransitView === "restrito" ? "Fluxo restrito" : "Fluxo completo"}</p></div>
  ...
  const renderHistoryList = () => filteredHistory.length === 0 ? (
    <div className="sig-dark-panel rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-600">Nenhum registro histórico encontrado com os filtros atuais.</div>
  ) : (
    <div className="space-y-3">
  ...
  return (
    <PortalFrame eyebrow="Controle de processos" title="Mesa institucional de tramitação">
      <PageShell className="sig-history-page sig-contrast-strong">
        ...
        <InternalTabs items={tabs as unknown as Array<{ value: string; label: string; helper?: string }>} value={activeTab} onChange={(value) => setActiveTab(value as ControlTab)} />
        ...
        {activeTab === "visao-geral" ? <PageMainGrid className="xl:grid-cols-[minmax(0,1.8fr)_minmax(320px,0.9fr)]"><PageMainContent><TableCard title="Caixas por unidade" description="Leitura institucional das pastas ativas, com recebidos, gerados e itens aguardando ação." icon={Workflow}><div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(320px,1fr))]">{processBoxes.map((box) => <div key={box.sector} className="sig-dark-panel h-full rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="sig-fit-title break-words text-sm font-semibold text-slate-950">{box.sector}</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="sig-dark-panel min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-3.5"><p className="sig-label break-words text-[10px] leading-tight">Recebidos</p><p className="mt-2 text-sm font-medium text-slate-900">{box.received}</p></div><div className="sig-dark-panel min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-3.5"><p className="sig-label break-words text-[10px] leading-tight">Gerados</p><p className="mt-2 text-sm font-medium text-slate-900">{box.generated}</p></div><div className="sig-dark-panel min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-3.5"><p className="sig-label break-words text-[10px] leading-tight">Aguardando</p><p className="mt-2 text-sm font-medium text-slate-900">{box.awaiting}</p></div></div></div>)}</div></TableCard><div className="grid gap-5 xl:grid-cols-2"><TableCard title="Recebidos da unidade" description="Fila principal da pasta atual." icon={Inbox}>{renderDispatchList(filteredReceived.slice(0, 4), "Nenhum despacho recebido para esta unidade.", true, true)}</TableCard><TableCard title="Atribuídos a mim" description="Pendências direcionadas ao usuário atual." icon={UserRoundCheck}>{renderDispatchList(assignedToMe.slice(0, 4), "Nenhum despacho atribuído diretamente a você.", true, true)}</TableCard></div></PageMainContent><PageSideContent><SectionCard title="Painel da unidade" description="Contexto rápido da pasta ativa, do modelo de despacho e da visibilidade do fluxo." icon={ShieldCheck}><div className="space-y-3"><div className="sig-dark-panel rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="sig-label">Unidade ativa</p><p className="mt-2 sig-fit-title break-words text-sm font-medium text-slate-900">{currentUnit}</p></div><div className="sig-dark-panel rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="sig-label">Fluxo restrito</p><p className="mt-2 text-sm font-medium text-slate-900">{restrictedProcesses.length} processo(s)</p><p className="mt-1 text-sm text-slate-500">Trâmites internos ocultos para o acesso externo.</p></div><div className="sig-dark-panel rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="sig-label">Texto padrão ativo</p><p className="mt-2 sig-fit-copy text-sm font-medium text-slate-900">{selectedTemplateId ? dispatchTemplates.find((item) => item.id === selectedTemplateId)?.title : "Nenhum texto padrão selecionado"}</p></div></div></SectionCard><SectionCard title="Prioridades de tramitação" description="Itens urgentes que pedem leitura ou resposta institucional." icon={Clock3}><div className="space-y-3">{criticalDispatches.length === 0 ? <div className="sig-dark-panel rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-600">Nenhum despacho prioritário agora.</div> : criticalDispatches.slice(0, 4).map((item) => <div key={item.id} className="sig-dark-panel rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="sig-fit-title text-sm font-medium text-slate-900">{item.protocol}</p><p className="mt-2 sig-fit-copy text-sm text-slate-600">{item.subject}</p><p className="mt-2 text-xs uppercase tracking-[0.18em] text-slate-400">{item.from} → {item.to}</p></div>)}</div></SectionCard><SectionCard title="Distribuição por responsável" description="Leitura de carga para coordenação e chefia." icon={UserRoundCheck}><div className="space-y-3">{responsibleSummary.length === 0 ? <div className="sig-dark-panel rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-600">Nenhum responsável atribuído no momento.</div> : responsibleSummary.map((item) => <div key={item.name} className="sig-dark-panel rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="sig-fit-title break-words text-sm font-medium text-slate-900">{item.name}</p><div className="mt-3 grid gap-2 sm:grid-cols-3"><div className="sig-dark-panel rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="sig-label">Total</p><p className="mt-2 text-sm font-medium text-slate-900">{item.total}</p></div><div className="sig-dark-panel rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="sig-label">Aguardando</p><p className="mt-2 text-sm font-medium text-slate-900">{item.pending}</p></div><div className="sig-dark-panel rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="sig-label">Críticos</p><p className="mt-2 text-sm font-medium text-slate-900">{item.critical}</p></div></div></div>)}</div></SectionCard></PageSideContent></PageMainGrid> : null}
