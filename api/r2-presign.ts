/**
 * api/r2-presign.ts — Vercel Serverless handler
 *
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    console.log("[ProdAudit][R2Presign] env", {
    ...
    json(res, 200, { uploadUrl, publicUrl, bucket, objectKey });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Erro ao gerar link de upload.";
